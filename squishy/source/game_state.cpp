...
void GameState::SetupStage_Level1() {
	...
	InitObject(kObj_Player, kSprites_squishyIdle1);
...
void GameState::Update_Level1() {
	...
	if (CheckKeyPress(KEY_LEFT)) {
		InitAnimation(&animatePlayer, kObj_Player, kSprites_squishyWalkLeft, kSprites_squishyWalkLeft4, 10); //set walking backwards animation
		SetAnimationFrame(&animatePlayer, 6);
	}	
	if (CheckKeyPress(KEY_RIGHT)) {
		InitAnimation(&animatePlayer, kObj_Player, kSprites_squishyWalkRight, kSprites_squishyWalkRight4, 10); //set walking backwards animation
		SetAnimationFrame(&animatePlayer, 10);
	}	
	if (CheckKeyPress(KEY_UP)) {
		InitAnimation(&animatePlayer, kObj_Player, kSprites_squishyWalkBack, kSprites_squishyWalkBack4, 10); //set walking backwards animation
		SetAnimationFrame(&animatePlayer, 18);
	}
	if (CheckKeyPress(KEY_DOWN)) {
		InitAnimation(&animatePlayer, kObj_Player, kSprites_squishyWalkFront, kSprites_squishyWalkFront4, 10); //set walking forwards animation
		SetAnimationFrame(&animatePlayer, 14);
	}
	...
	if (dx != 0 || dy != 0) {
        ...
        if (x < -3 || x > SCREEN_WIDTH - width + 3 )
            dx = 0;
        if (y < -12 || y > SCREEN_HEIGHT - height)
            dy = 0;
	...
	else //when the player is not moving the idle animation needs to be active
	{
	InitAnimation(&animatePlayer, kObj_Player, kSprites_squishyIdle1, kSprites_squishyIdle6, 10);
	SetAnimationFrame(&animatePlayer, 0); //makes player face forward
	}
